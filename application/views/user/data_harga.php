...
<body>
        ...
        <ul>
          ...
          <li class="active drop-down"><a href="#datapasar">DATA PASAR</a>
            <ul>
              ...
              <li><a href="#data_harga">Data Harga Komoditas</a></li>
            </ul>
          </li>
    ...
    <div class="container">
      <div class="row">
        <div class="col-lg-6 order-1 order-lg-2 hero-img" data-aos="fade-left" data-aos-delay="200">
          <img src="<?=base_url()?>assets/components/icon/graphic.svg" class="img-fluid animated" width="50%" alt="">
        </div>
        <div class="col-lg-6 pt-5 pt-lg-0 order-2 order-lg-1 d-flex flex-column justify-content-center">
          <h1 data-aos="fade-up">Data Harga Komoditas</h1>
          <h2 data-aos="fade-up" data-aos-delay="400">Informasi perkembangan harga komoditas <br>Unit Pelaksana Teknis Daerah Pasar Jatirogo kurun waktu 5 hari terakhir.</h2>
        </div>
      </div>
    </div>
  ...
  <main id="main">
    <!-- ======= data pegawai ======= -->
    <section id="data_harga" class="datapasar">
      <div class="container">
              ...
              <table id="table1" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    ...
                    <th class="align-middle" rowspan="3">Nama Komoditas</th>
                    <th class="align-middle" rowspan="3">Satuan</th>
                    <th class="text-center" colspan="10">Tanggal</th>
                    <tr>
                      <th class="text-center" colspan="2"> <?= date("d M Y", strtotime("-4 days")) ?></th>
                      <th class="text-center" colspan="2"> <?= date("d M Y", strtotime("-3 days")) ?></th>
                      <th class="text-center" colspan="2"> <?= date("d M Y", strtotime("-2 days")) ?></th>
                      <th class="text-center" colspan="2"> <?= date("d M Y", strtotime("-1 days")) ?></th>
                      <th class="text-center" colspan="2"> <?= date("d M Y") ?></th>
                    </tr>
                ...
                <tbody>
                  <?php 
                    ...
                    $data = $this->M_data_harga->getDatanew();
                    // print_r($datanew);
                    // print_r($data);
                    // $datanew = $this->M_data_harga->getData();
                    // print_r($datanew);
                    foreach($data as $dt): 
                    ?>
                          <tr>
                              ...
                              <td><?= $dt['SATUAN'] ?></td>
                              <td><?= min(explode(',' ,$dt[date('Y-m-d', strtotime("-4 days"))])) ?></td>
                              <td><?= max(explode(',' ,$dt[date('Y-m-d', strtotime("-4 days"))])) ?></td>
                              <td><?= min(explode(',' ,$dt[date('Y-m-d', strtotime("-3 days"))])) ?></td>
                              <td><?= max(explode(',' ,$dt[date('Y-m-d', strtotime("-3 days"))])) ?></td>
                              <td><?= min(explode(',' ,$dt[date('Y-m-d', strtotime("-2 days"))])) ?></td>
                              <td><?= max(explode(',' ,$dt[date('Y-m-d', strtotime("-2 days"))])) ?></td>
                              <td><?= min(explode(',' ,$dt[date('Y-m-d', strtotime("-1 days"))])) ?></td>
                              <td><?= max(explode(',' ,$dt[date('Y-m-d', strtotime("-1 days"))])) ?></td>
                              <td><?= min(explode(',' ,$dt[date('Y-m-d')])) ?></td>
                              <td><?= max(explode(',' ,$dt[date('Y-m-d')])) ?></td>
                          </tr>
                  <?php endforeach;?>
                </tbody>
              </table>
